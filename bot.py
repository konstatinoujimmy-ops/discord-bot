            ...
            if invites_channel:
                ...
                try:
                    async with asyncio.timeout(300):  # 5 min timeout
                        async for msg in invites_channel.history(limit=None):
                            message_count += 1
                            if msg.mentions:
    ...
    try:
        ...
        async for member in guild.fetch_members(limit=None):
            ...
            try:
                ...
                # Rate limiting: 0.2 seconds ανάμεσα σε κάθε μέλος (~4 λεπτά για 1200 members - super fast!)
                await asyncio.sleep(0.2)
...
def run_bot():
    # Use new token (third bot - DISCORD_TOKEN_2)
    token = os.getenv('DISCORD_TOKEN_2')
    
    if not token:
        logger.error("❌ DISCORD_TOKEN_2 not found in environment variables!")
        return
